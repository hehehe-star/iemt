...
class Model(nn.Module):
    ...
    def forward(self, name_emb1, type_emb1, coor_emb1,name_emb2, type_emb2, coor_emb2):#结果要是一个tensor 六个输入
        # -------------直接拼接+ cat + lineer + sigmoid-----------
        # ------------（1）
        topn1 = torch.cat((name_emb1,type_emb1,coor_emb1),dim=1)
        topn2 = torch.cat((name_emb2,type_emb2 ,coor_emb2), dim=1)
        result = torch.cat((topn1,topn2),dim=1)
        output = self.linner4192(result)
        ...
        # -------------直接相加 + lineer + sigmoid-----------
        # ------------（2）
        # coor_emb1 = self.coor_linner(coor_emb1)
        # coor_emb2 = self.coor_linner(coor_emb2)
        # topn1 = name_emb1+type_emb1+coor_emb1
        # topn2 = name_emb2+type_emb2+coor_emb2
        # result = torch.cat((topn1,topn2),dim=1)
        # output = self.linner20481(result)
        # output = self.sigmoid(output)
        # return output

        # #-------------仅文本特征输入融合模块，后与空间特征进行拼接 + cat + lineer + sigmoid-----------
        # #------------（3）
        # text1 = torch.cat((name_emb1, type_emb1),dim=1).unsqueeze(-1).unsqueeze(-1)
        # text2 = torch.cat((name_emb2, type_emb2),dim=1).unsqueeze(-1).unsqueeze(-1)
        # text1 = self.oneFusion(text1).squeeze()
        # text2 = self.oneFusion(text2).squeeze()
        # topn1 = torch.cat((text1,coor_emb1),dim=1)
        # topn2 = torch.cat((text2,coor_emb2),dim=1)
        #
        # result = torch.cat((topn1,topn2),dim=1)
        # output = self.linner4192(result)
        # output = self.sigmoid(output)
        ...
        # #-------------仅空间特征输入融合模块，后与文本特征进行拼接 + cat + lineer + sigmoid-----------
        # #------------（4）
        # coor1 = self.oneFusion(coor_emb1.unsqueeze(-1).unsqueeze(-1)).squeeze()
        # coor2 = self.oneFusion(coor_emb2.unsqueeze(-1).unsqueeze(-1)).squeeze()
        # topn1 = torch.cat((name_emb1,type_emb1,coor1),dim=1)
        # topn2 = torch.cat((name_emb2,type_emb2,coor2),dim=1)
        # result = torch.cat((topn1,topn2),dim=1)
        # output = self.linner4192(result)
        # output = self.sigmoid(output)
        # return output

        #-------------文本特征和空间特征分别单独输入融合模块后，再进行拼接 + cat + lineer + sigmoid-----------
        #------------（5）
        # coor_emb1 = self.coor_linner(coor_emb1)
        # coor_emb2 = self.coor_linner(coor_emb2)
        #
        ...
        # coor2 = self.oneFusion(coor_emb2.unsqueeze(-1).unsqueeze(-1)).squeeze()
        #
        # text1 = name_emb1+type_emb1
        # text2 = name_emb2+type_emb2
        # text1 = self.oneFusion(text1.unsqueeze(-1).unsqueeze(-1)).squeeze()
        # text2 = self.oneFusion(text2.unsqueeze(-1).unsqueeze(-1)).squeeze()
        #
        # topn1 = torch.cat((text1,coor1),dim=1)
        # topn2 = torch.cat((text2,coor2),dim=1)
        # result = torch.cat((topn1,topn2),dim=1)
        # output = self.linner4096(result)
        # output = self.sigmoid(output)
        # return output

        # #-------------文本特征和空间特征分别单独输入融合模块后，再进行相加 + cat + lineer + sigmoid-----------
        # #------------（6）
        # coor_emb1 = self.coor_linner(coor_emb1)
        # coor_emb2 = self.coor_linner(coor_emb2)
        #
